...
p_list <- seq(0.1, 0.5, by = 0.1)
n_reps <- 10
outer_output_list <- list()
outer_output_list_cells <- list()
z <- 0

for(p in p_list){
  print(paste("Progress ---> p = ", p, " of ", max(p_list), sep = ""))
  for(ps in ps_list){
    ...
    for(k in 1:n_reps){
      ...
      set.seed(42 + z)
      input_landscape <- generate_inputs_from_random_landscape(nrow = cell_n, ncol = cell_n, resolution = cell_res,
                                                               prop_cluster = p, prop_structure = ps)
      ...
      #Detect if the random landscape has 0 or 1 structure cells
      if(sum(landscape_dat[,1]) == 0 | sum(landscape_dat[,1]) == 1){
    ...
    remake(prob, p = VectorOfArray([Rc, Rm, pm, Gtc, Gtv, Gti, y, Dc, Dv_list[i], Di, w, neighbor_mat_m, neighbor_mat_c, dispersal_mat, C_growth, Mi_growth, Mi_disp, turf]))
      ...
      for(j in 1:2){
        ...
        # start.time <- Sys.time()
        # sol <- de$solve(ensembleprob,de$Tsit5(),de$EnsembleThreads(),trajectories=2)
        # end.time <- Sys.time()
        # time.taken <- round(end.time - start.time,2)
        # print(time.taken)
        ...
        for(i in 1:length(Dv_list)){
          ...
          temp_frame <- data.frame("rand.seed" = z+42,
                                   ...
                                   "prop_structure" = ps,
                                   "res" = cell_res,
                                   "size" = length(landscape_dat[,1]),
                                   "rep" = k,
                                   "p_struc" = sum(landscape_dat[,1] == 1) / length(landscape_dat[,1]),
          ...
          temp_cell_output <- data.frame("rand.seed" = rep(z+42, times = n_cells),
                                         "prop_cluster" = rep(p, times = n_cells),
      ...
      time.taken <- round(end.time - start.time,2)
      print(paste("p = ", p, " took ", time.taken, sep = ""))
      
      #####################
      #CONVERTS LIST OUTPUTS TO DATAFRAME
      #####################
      
      outer_output_list[[paste("p", p, "_ps", ps, "_rep", k, sep = "")]] <- output_list
      outer_output_list_cells[[paste("p", p, "_ps", ps, "_rep", k, sep = "")]] <- cell_outputs_list
      
      #Saves cell level analysis as we're going along in case Windows updates...
...
#Saves cell-level analysis
#saveRDS(outer_output_list_cells, file = "outputs\\cell_outputs_cluster.rds")

#Summarizing the hysteresis outputs from list format into dataframes for saving
...
hysteresis_outputs$unique_id_rep <- paste(hysteresis_outputs$prop_structure,
                                          hysteresis_outputs$prop_cluster,
...
#write.csv(hysteresis_outputs, file = "cluster_outputs.csv", row.names = F)